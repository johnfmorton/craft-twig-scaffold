 ...
 *
 * @phpstan-type NeoType array{id: int, handle: string, name: string, layout: FieldLayout, topLevel: bool, children: list<string>|'*'}
 * @phpstan-type NeoField array{types: array<string, NeoType>, maxLevels: int|null, budget: int}
 * @phpstan-type Partial array{key: string, label: string, path: string, variable: string, entryType: EntryType|null, neo: NeoType|null, children: list<string>}
 */
class Generator extends Component
{
    ...
    /**
     * The partial templates the template being written relies on, in the
     * order they were first needed. Only filled in partial-templates mode;
     * building one partial can queue more.
     *
     * @var list<Partial>
     */
    private array $partialQueue = [];

    /**
     * @var array<string, true> the paths already in {@see self::$partialQueue}. Craft
     * resolves a partial by path, so two block types with the same handle share one.
     */
    ...
    {
        return $this->scaffold($entryType, $matrixMode)['twig'];
    }

    /**
     * Returns a starter template for the entry type and, in partial-templates
     * mode, a starter for every partial template it relies on: one per Matrix
     * or Super Table entry type and per Neo block type, including those only
     * reached through another partial.
     *
     * @param EntryType $entryType
     * @param string $matrixMode One of the `MATRIX_*` constants
     * @return array{twig: string, partials: list<array{label: string, path: string, variable: string, twig: string}>}
     */
    public function scaffold(EntryType $entryType, string $matrixMode = self::MATRIX_INLINE): array
    {
        ...
        $partials = [];
        // Writing a partial can queue more (a Matrix field inside a Neo block), so walk by index.
        for ($i = 0; $i < count($this->partialQueue); $i++) {
            $partials[] = $this->partialTemplate($this->partialQueue[$i]);
        }

        return ['twig' => $twig, 'partials' => $partials];
    }

    /**
     * Writes one partial template: a header, the provider's fields, and for a
     * Neo block type that allows children, the call that renders them.
     *
     * @param Partial $partial
     * @return array{label: string, path: string, variable: string, twig: string}
     */
    private function partialTemplate(array $partial): array
    {
        $var = $partial['variable'];
        $lines = [
            '{#',
            self::INDENT . self::comment($partial['label']),
            self::INDENT . "Partial template Craft uses to render each block of this type; the block is available as `{$var}`.",
            self::INDENT . 'Starter template generated by Twig Scaffold. Edit freely; it is a starting point, not a finished template.',
            self::INDENT . 'Suggested location: ' . $partial['path'],
            '#}',
            '',
        ];

        if ($partial['entryType'] !== null) {
            $entryType = $partial['entryType'];
            if ($entryType->hasTitleField) {
                $lines[] = "<h2>{{ {$var}.title }}</h2>";
            }
            $this->appendLayout($lines, $entryType->getFieldLayout(), $var, 0, [$var], [$partial['key']]);
        } elseif ($partial['neo'] !== null) {
            $this->appendLayout($lines, $partial['neo']['layout'], $var, 0, [$var], [$partial['key']]);
            if ($partial['children'] !== []) {
                $lines[] = '';
                $lines[] = '{# Child blocks render with the partial templates for their types: ' . implode(', ', $partial['children']) . ' #}';
                $lines[] = "{{ {$var}.children.render() }}";
            }
        }
    ...
    /**
     * Queues the partial template for an entry type, once, and returns its path.
     */
    private function queueEntryTypePartial(EntryType $entryType): string
    {
        $path = $this->partialPath($entryType);
        $key = 'entryType:' . $entryType->id;
        if (!isset($this->partialKeys[$path])) {
            $this->partialKeys[$path] = true;
            $this->partialQueue[] = [
                'key' => $key,
                'label' => "{$entryType->name} (entry type \"{$entryType->handle}\")",
                'path' => $path,
                'variable' => 'entry',
                'entryType' => $entryType,
                'neo' => null,
    ...
    /**
     * Queues the partial template for a Neo block type, once, and returns its path.
     *
     * @param NeoType $type
     * @param array<string, NeoType> $types every block type of the field, for the children
     */
    private function queueNeoPartial(ElementContainerFieldInterface $field, array $type, array $types): string
    {
        $path = $this->neoPartialPath($type);
        $key = 'blockType:' . $type['id'];
        if (!isset($this->partialKeys[$path])) {
            $this->partialKeys[$path] = true;
            $this->partialQueue[] = [
                'key' => $key,
                'label' => "{$type['name']} (Neo block type \"{$type['handle']}\" in {$field->name})",
                'path' => $path,
                'variable' => self::refHandleFor($type['layout'], 'neoblock'),
                'entryType' => null,
                'neo' => $type,
                'children' => array_map(fn(array $child) => $this->neoPartialPath($child), self::neoChildTypes($type, $types)),
            ];
        }
     ...
     * @param NeoType $type
     */
    private function neoPartialPath(array $type): string
    {
        return $this->partialPathFor(self::refHandleFor($type['layout'], 'neoblock'), $type['handle']);
    }
    ...
    private function appendNeo(array &$lines, ElementContainerFieldInterface $field, string $expr, int $depth, array $variables, array $ancestors): void
    {
        ...
        if ($this->matrixMode === self::MATRIX_PARTIALS) {
            $partials = array_map(fn(array $type) => $this->queueNeoPartial($field, $type, $types), array_values($types));
            $variable = self::refHandleFor($top[0]['layout'], 'neoblock');
            $lines[] = "{$indent}{# Renders each block with the partial template for its type, where the block is available as `{$variable}`"
                . ($hierarchical ? " and its children render with {{ {$variable}.children.render() }}" : '')
                . ': ' . implode(', ', $partials) . ' #}';
    ...
    private function neoBlockTypes(ElementContainerFieldInterface $field): array
    {
        ...
        foreach ($field->getFieldLayoutProviders() as $provider) {
            ...
            $name = self::attribute($provider, 'name');
            $types[$handle] = [
                'id' => (int)self::attribute($provider, 'id'),
                'handle' => $handle,
                'name' => is_string($name) && $name !== '' ? $name : $handle,
